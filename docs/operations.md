...
`thread-chain-index-lab` is centered on blockchain tooling work in C. The technical target is: Implement a C blockchain tooling project for index state machine modeling, using transition tables and invalid-transition tests.
...
The score is deterministic. The current threshold is 158. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 2.
